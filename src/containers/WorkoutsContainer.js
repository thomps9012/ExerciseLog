...
class WorkoutsContainer extends Component {
    ...
    componentDidMount() {
        ...
        workoutsRef.once('value', (list) => {
            list.forEach((data) => {
                ...
                initWorkouts.push({
                    id: workout.id,
                    ref: data.key,
                    date: moment.unix(workout.date).formate('YYYY-MM-DD'), //convert timestap
        ...
        workoutsRef.on('child_added', (data) => {
            ...
            this.addWorkout({
                ...workout,
                date: moment.unix(workout.date).format('YYYY-MM-DD')
            });
        });
    ...
    addWorkout({ id, ref, date = false, exercises = [] }) {
        date = date || moment().formate('YYYY-MM-DD');
